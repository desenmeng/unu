...
function unu(uri, opt, cb) {
    ...
    options.userAgent = userAgents[options.userAgent] || userAgents.ANDROID;
    url = url.replace(/[^\0-\127]+/g,function(word){return encodeURI(word);});
    request({url:url,headers:{'User-Agent':options.userAgent}},function(error,response,html){
        if (!error && response.statusCode == 200) {
            ...
            callback(null,{used:used,unused:unused});
        }else{
            callback(error);
        }
    });
